...
def search_identity(
    ...
    if len(target_objs) == 1:
        ...
        target_img = cv2.resize(
            target_img, (IDENTIFIED_IMG_SIZE, IDENTIFIED_IMG_SIZE))
        target_img *= 255
...
def overlay_identified_face(
    ...
    try:
        if y - IDENTIFIED_IMG_SIZE > 0 and x + w + IDENTIFIED_IMG_SIZE < img.shape[1]:
            ...
            # top left
            img[y - IDENTIFIED_IMG_SIZE: y, x -
                IDENTIFIED_IMG_SIZE: x] = target_img
...
def overlay_emotion(
    ...
    """
    emotion_df = pd.DataFrame(emotion_probas.items(),
